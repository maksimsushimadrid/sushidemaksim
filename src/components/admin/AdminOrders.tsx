...
const ORDERS_TRANSLATIONS = {
    ru: {
        ...
        statusNamesRecogida: {
            ...
            confirmed: 'Заказ подтвержден',
            preparing: 'Передан кухне',
            on_the_way: 'Готов и ожидает клиента',
...
const getWhatsAppConfirmationUrl = (order: Order, isPickup: boolean) => {
    ...
    let text = '';
    if (scheduledTime) {
        text = `¡Hola! Hemos recibido tu pedido #${String(order.id).padStart(5, '0')}${scheduledText ? `\r\n${scheduledText}` : ''}\r\n\r\n${itemsList}\r\n\r\n`;
    } else {
        const deliveryStatusMsg = isPickup
            ? 'Aproximada hora de recogida en 30 - 45 minutos.'
            : 'Te lo entregaremos en 30 - 60 minutos.';
        text = `¡Hola! Hemos recibido tu pedido. ${deliveryStatusMsg}\r\n\r\nTu pedido #${String(order.id).padStart(5, '0')} está confirmado\r\n\r\n${itemsList}\r\n\r\n`;
    }
    if (!isPickup) {
                ...
                <div className="grid gap-6">
                    {isLoading && orders.length === 0 ? (
                        ...
                        orders.map((order: Order) => {
                            const notesForType = order.notes || '';
                            let parsedDeliveryType = '';
                            const typeParts = notesForType.split(' | ');
                            typeParts.forEach((part: string) => {
                                if (part.includes('[TIPO:')) {
                                    parsedDeliveryType = part
                                        .replace('[TIPO: ', '')
                                        .replace('[TIPO:', '')
                                        .replace(']', '')
                                        .trim();
                            ...
                            const isPickup =
                                !isMesa &&
                                (parsedDeliveryType === 'RECOGIDA EN LOCAL' ||
                                            ...
                                            <div className="p-5 sm:p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 overflow-hidden">
                                                ...
                                                <div className="lg:border-l-2 border-dashed border-gray-100 lg:pl-10 flex flex-col justify-start">
                                                    ...
                                                    {(order.status === 'pending' ||
                                                        ...
                                                        >
                                                            <ChefHat size={18} strokeWidth={2.5} />A
                                                            LA COCINA
                                                        </motion.button>
                                                    )}
